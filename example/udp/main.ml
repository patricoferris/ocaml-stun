...
let packet random =
  Packet.(
    create ~random ~typ:Message.(Binding Request) ~payload:Cstruct.empty ())

let main ~sw net random =
  info "Starting stun connection...";
  let client = Client.create ~uri:(Uri.of_string "stun.l.google.com") 19302 in
  let conn = Client.init ~sw net client in
  info "Connection established...";
  let buff = Cstruct.create 80 in
